...
ModelBase.prototype.model = function model (source, initialValue) {
  if (isConsumable(source)) {
    var consumable = source;
    return this.hook(function () { return consumable.consume(); });
  } else {
    return arguments.length > 1 ? new source(initialValue) : new source();
  }
};
...
function createStore(modelClass) {
  ...
  var Provider = function (props) {
    var createModel = 'initialValue' in props ? function () { return new modelClass(props.initialValue); } : function () { return new modelClass(); };
...
function useModel(source, initialValue) {
  if (isConsumable(source)) {
    var consumable = source;
    return consumable.consume();
  } else {
    var createModel = arguments.length > 1 ? function () { return new source(initialValue); } : function () { return new source(); };
    return resolveModel(createModel);
  }
}

export { ModelBase, PureModel, Model, createStore, useModel };
//# sourceMappingURL=react-model-store.mjs.map
